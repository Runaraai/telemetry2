...
class ScalewayLaunchRequest(BaseModel):
    ...
    root_volume_size: Optional[int] = Field(20_000_000_000, description="Root volume size in bytes (default 20GB)")
    root_volume_type: Optional[str] = Field("l_ssd", description="Root volume type")
...
async def _ensure_ssh_key(client: httpx.AsyncClient, headers: dict, project_id: str, public_key: str, name: Optional[str]) -> str:
    ...
    if resp.status_code == 201:
        data = resp.json()
        return data.get("id")
    # If already exists or conflict, try to find it
    if resp.status_code in (400, 409):
        list_resp = await client.get("https://api.scaleway.com/iam/v1alpha1/ssh-keys", headers=headers, params={"project_id": project_id})
        if list_resp.status_code == 200:
            for item in list_resp.json().get("ssh_keys", []):
                if item.get("name") == key_name or item.get("public_key") == public_key:
                    return item.get("id")
